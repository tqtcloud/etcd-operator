...
import (
	"github.com/tqtcloud/etcd-operator/api/v1beta1"
...
var (
	EtcdClusterLabelKey       = "etcd.tqtcloud.io" // Selector 标签key
	EtcdClusterCommonLabelKey = "app"              //Labels key
	EtcdDataDirName           = "datadir"          // VolumeClaimTemplates  Name
)

// MutateStatefulSet  构造 sts 对象
func MutateStatefulSet(cluster *v1beta1.EtcdCluster, sts *appsv1.StatefulSet) {
	sts.Labels = map[string]string{ //构造标签
...
func NewContainers(cluster *v1beta1.EtcdCluster) []corev1.Container {
	return []corev1.Container{
		{
			...
			Env: []corev1.EnvVar{
				corev1.EnvVar{
					...
					Value: strconv.Itoa(int(*cluster.Spec.Size)), //将 数值转换为字符串
				},
				corev1.EnvVar{
					Name:  "SET_NAME",
					Value: cluster.Name,
...
func MutateHeadlessSvc(cluster *v1beta1.EtcdCluster, svc *corev1.Service) {
